...
public static class SmoothBackgroundTransitionBehavior
{
    ...
    public static bool GetEnableSmoothTransition(DependencyObject obj)
    {
        return (bool)obj.GetValue(EnableSmoothTransitionProperty);
    }

    public static void SetEnableSmoothTransition(DependencyObject obj, bool value)
    {
        obj.SetValue(EnableSmoothTransitionProperty, value);
    }

    private static Brush GetPreviousBrush(DependencyObject obj)
    {
        return (Brush)obj.GetValue(PreviousBrushProperty);
    }

    private static void SetPreviousBrush(DependencyObject obj, Brush value)
    {
        obj.SetValue(PreviousBrushProperty, value);
    }
    ...
    private static void OnBackgroundChanged(object sender, EventArgs e)
    {
        ...
        SolidColorBrush animBrush = new(startColor);

        // Cria a animação para a propriedade Color
        ColorAnimation animation = new(endColor, TimeSpan.FromMilliseconds(300))
        {
